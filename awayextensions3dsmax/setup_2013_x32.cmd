@echo off
echo.
set programmname=3dsmax 2013 32 bit
echo AwayExtensions-3dsmax Installer for %programmname%
echo.
echo.
echo Detected pathes:
echo.
set programmpath=%ProgramFiles%\Autodesk\3ds Max 2013\

iF DEFINED ProgramFiles(x86) (set programmpath="%ProgramFiles(x86)%\Autodesk\3ds Max 2013\")

IF DEFINED ADSK_3DSMAX_x32_2013 (set programmpath="%ADSK_3DSMAX_x32_2013%")

set programmpath=%programmpath:"=%
set exportername=maxawdexporter_2013_32.dle
set toolbarname=AWDToolBar_2013_2014.cuix
set uninstallername=uninstall_awayextensions3dsmax_2013_x32.cmd
...
set localuserpath=%LOCALAPPDATA%\Autodesk\3dsMax\2013 - 32bit\enu\
...
set iconsPath=%localuserpath%usericons\
if EXIST "%iconsPath%" (echo  - usericon:   local-user-path + "\usericons") ELSE (goto exitNoUserIcons)
set uiPath=%programmpath%en-US\UI\
...
:install
   ...
   if EXIST "%iconsPath%\AWDAbout.bmp" (del /q "%iconsPath%\AwayExtensionIcons_24i.bmp")
   ...
   xcopy /q/s/e/y/x "AwayExtensions3dsMax" "%pluginsPath%"
   echo.
   echo Copy File 'AWDExporter\%exportername%' to the plugin folder
   xcopy /q/y/x "AWDExporter\%exportername%" "%pluginsPath%"
   echo.
